...
class TestPS5(unittest.TestCase):
    def test_generate_models(self):
        ...
        self.assertListEqual(
            list(models[0]), list(pylab.polyfit(x, y, 1)), coefficient_mismatch
        )
    ...
    def test_moving_avg(self):
        ...
        self.assertListEqual(
            list(result), list(correct), "Moving average values incorrect"
        )

        y = [-1.5, 1.5, -3.0, 3.0, -4.5, 4.5]
        window_length = 2
        correct = [-1.5, 0, -0.75, 0, -0.75, 0]
        result = ps5.moving_average(y, window_length)
        self.assertListEqual(
            list(result), list(correct), "Moving average values incorrect"
        )
    ...
    def test_gen_std_devs(self):
        ...
        for index in range(len(correct)):
            good_enough = math.isclose(correct[index], result[index])
            self.assertTrue(
                good_enough, "Standard deviations do not match expected results"
            )
